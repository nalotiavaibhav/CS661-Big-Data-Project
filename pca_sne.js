async function createPlot(data, state) {
  ...
  var colorScale = d3v7
    ...
    .range(["green", "yellow", "orange", "red"]); // Adjust color range as needed
  ...
  svg
    ...
    .attr("fill", function (d, i) {
      var color = colorScale(air_quality_data[i]);
      return color;
    })
    .attr("opacity", 0.7);
...
function updateMethod() {
  ...
  if (method == "PCA") {
    createPlot(pcaData, selectedState);
  } else {
    createPlot(sneData, selectedState);
  }
}
