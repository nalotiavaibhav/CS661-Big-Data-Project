...
function d3_timeseries() {
  ...
  var _tipFunction = function (date, series) {
    ...
    return "<h4>" + xscale.setformat(d3v5.timeDay(date)) + "</h4>" + spans;
  };

  function createLines(serie) {
    // https://github.com/d3v5/d3v5-shape/blob/master/README.md#curves
    var aes = serie.aes;
    ...
    // to uppercase for d3v5 curve name
    var curveName =
      "curve" +
      serie.options.interpolate[0].toUpperCase() +
      serie.options.interpolate.slice(1);
    serie.interpolationFunction = d3v5[curveName] || d3v5.curveLinear;
    ...
    serie.find = function (date) {
      var bisect = d3v5.bisector(fk(aes.x)).left;
  ...
  function mouseMove() {
    var x = d3v5.mouse(container.node())[0];
    x = xscale.invert(x);
  ...
  var chart = function (elem) {
    ...
    series = series.map(function (s) {
      var extent = d3v5.extent(s.data.map(functorkey(s.aes.y)));
      s.min = extent[0];
      s.max = extent[1];
      extent = d3v5.extent(s.data.map(functorkey(s.aes.x)));
      s.dateMin = extent[0];
      s.dateMax = extent[1];
      return s;
    });
    ...
    yscale
      ...
      .domain([
        d3v5.min(series.map(fk("min"))),
        d3v5.max(series.map(fk("max"))),
      ])
      .nice();
    ...
    xscale
      ...
      .domain([
        d3v5.min(series.map(fk("dateMin"))),
        d3v5.max(series.map(fk("dateMax"))),
      ])
      .nice();
    ...
    drawerContainer = svg
      ...
      .attr(
        ...
        "translate(" +
          margin.left +
          "," +
          (height - drawerHeight - margin.bottom) +
          ")"
      );
    ...
    mousevline.update = function () {
      this.attr("transform", function (d) {
        return (
          "translate(" + (margin.left + xscale(d.x)) + "," + margin.top + ")"
        );
    ...
    var xAxis = d3v5.axisBottom().scale(xscale).tickFormat(xscale.setformat);
    var yAxis = d3v5.axisLeft().scale(yscale).tickFormat(yscale.setformat);
    ...
    brush
      ...
      .on("brush", () => {
        let selection = d3v5.event.selection;
    ...
    svg
      ...
      .attr("x", -margin.top - d3v5.mean(yscale.range()))
      .attr("dy", ".71em")
  ...
  d3v5.keys(margin).forEach(function (k) {
...
function draw_timeSeries(state, selectedPollutant) {
  console.log(selectedPollutant, state);

  d3v5.csv("filtered_pollution_data.csv").then(function (data) {
    var parseTime = d3v5.timeParse("%d-%m-%Y %H:%M");
    ...
    function updateChart() {
      ...
      var currentPollutant = selectedPollutant;
      var currentData = data.filter((d) => d.State === currentState);
      if (chart) {
        // console.log(chart);
        d3v5.select("#chart").remove();
      }
    ...
    // d3v5.select("#pollutant-select").on("change", updateChart);
